...
| `odoo_accounting` (create_invoice) | Financial Action (Gold Tier) |
| `odoo_accounting` (log_payment) | Financial Action (Gold Tier) |

### MCP Actions That Do NOT Require Approval
| MCP Tool | Reason |
|----------|--------|
| `check_email_config` | Read-only, no side effects |
| `odoo_accounting` (get_report) | Read-only, no side effects (Gold Tier) |
